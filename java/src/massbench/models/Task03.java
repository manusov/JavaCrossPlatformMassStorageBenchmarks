...
import massbench.task03.SampleChannels;

public class Task03 extends Command 
{
//---------- Object name support -----------------------------------------------

private final String name = "Channels file input-output";
...
public Task03()
    ...
    opb = new Parm[]   // Assign Output Parameters Block (OPB) 
        ...
        new ParmDouble( "minimum" , "MBPS" , 0.0 ) ,
        new ParmDouble( "maximum" , "MBPS" , 0.0 ) ,
        new ParmDouble( "COPY speed, median" , "MBPS" , 0.0 ) ,
        new ParmDouble( "average" , "MBPS" , 0.0 ) ,
        new ParmDouble( "minimum" , "MBPS" , 0.0 ) ,
        new ParmDouble( "maximum" , "MBPS" , 0.0 ) ,
        };
...
@Override public STATUS execute()
    ...
    String s, fullname, fullnameSrc, fullnameDst;
    SampleChannels sch = new SampleChannels();
    
    //--- Built buffer, store speed ---
    System.out.println("\r\n[INITIALIZING]");
    double mbps = sch.builtBuffer( dataArray );
    ...
    double[] copyMBPS = new double[count];
    ...
    if ( mode != 1 )              // skip write if read only
    {
        ...
        for ( int i=0; i<count; i++ )
            {
            fullname = fname + "_src_" + i + fext;
            mbps = sch.writeFile( fullname, blocks, cacheMode );
            s = String.format
    ...
    if ( mode != 2 )              // skip read and delete if write only
        ...
        for ( int i=0; i<count; i++ )
            {
            fullname = fname + "_src_" + i + fext;
            mbps = sch.readFile( fullname , blocks );
            s = String.format
                ( "Read %s , %.3f MBPS" , fullname, mbps );
            System.out.println(s);
            readMBPS[i] = mbps;
            }
        //--- Copy files with pre-pause ---        
        Timings.timeDelay(rpause);
        System.out.println("\r\n[COPY]");
        for ( int i=0; i<count; i++ )
            {
            fullnameSrc = fname + "_src_" + i + fext;
            fullnameDst = fname + "_dst_" + i + fext;
            mbps = sch.copyFile( fullnameSrc, fullnameDst, blocks, cacheMode );
            s = String.format
                ( "Copy %s , %.3f MBPS" , fullnameSrc+" - "+fullnameDst, mbps );
            System.out.println(s);
            copyMBPS[i] = mbps;
            }
        ...
        for ( int i=0; i<count; i++ )
            {
            fullname = fname + "_src_" + i + fext;
            sch.deleteFile( fullname );
            fullname = fname + "_dst_" + i + fext;
            sch.deleteFile( fullname );
            }
    ...
    ((ParmDouble)opb[9]).value  = calcMedian(copyMBPS);
    ((ParmDouble)opb[10]).value = calcAverage(copyMBPS);
    ((ParmDouble)opb[11]).value = calcMinimum(copyMBPS);
    ((ParmDouble)opb[12]).value = calcMaximum(copyMBPS);
    ...
    if (mode==1)                       // reject write+copy results if ro
        ...
        ((ParmDouble)opb[9]).valid = false;
        ((ParmDouble)opb[10]).valid = false;
        ((ParmDouble)opb[11]).valid = false;
        ((ParmDouble)opb[12]).valid = false;
        }
